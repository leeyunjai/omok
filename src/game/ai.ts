...
function minimax(
  ...
  if (depth === 0 || moves.length === 0) {
    return moves.slice(0,5).reduce((s,{row,col}) => {
      const b = cloneBoard(board); b[row][col] = ai; return s + evalPos(b,row,col,ai);
    }, 0);
  }
  const cur = maxing ? ai : (ai==='black'?'white':'black');
  let best = maxing ? -Infinity : Infinity;
  for (const {row,col} of moves) {
    const nb = cloneBoard(board); nb[row][col] = cur;
...
export function getBestMove(board: BoardState, ai: PieceColor, difficulty: Difficulty): Position | null {
  ...
  for (const {row,col} of moves.slice(0, limit)) {
    const nb = cloneBoard(board); nb[row][col] = ai;
