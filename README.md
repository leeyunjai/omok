...
`base`가 상대 경로(`./`)라서 계정 이름이나 저장소 이름, 배포 경로가 바뀌어도 그대로 동작합니다.
매니페스트와 서비스 워커의 경로도 전부 상대 경로라 소스에 주소가 박혀 있는 곳이 없습니다.

다만 주소가 바뀌면 **오리진이 바뀝니다.** localStorage는 오리진마다 따로라서
이전 주소에서 쌓은 기록·이어하기와 설치해 둔 PWA는 새 주소로 따라오지 않습니다.
